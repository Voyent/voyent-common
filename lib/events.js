...
function createEvent(account, realm, service, event, type, username, data, time, txcode) {
    ...
    var time = time ? time : new Date().toISOString();
...
function createCustomEventFromBarrel(barrel) {
    ...
    var resources = barrel.validatedResource;
    ...
    var time = resources.time ? resources.time : new Date().toISOString();
