...
//console.log('db.js', 'dbURLPrefix', dbURLPrefix);
...
function testDatabaseExistence(accountName, cb) {
    ...
    getDatabaseList(function (err, dbs) {
        for (var i = 0; i < dbs.length; i++) {
            logger.debug('------> Comparing: ' + dbs[i].name + ' with: ' + safeDB);
        ...
        logger.debug('No Exiting database found: [' + safeDB + ']');
        return cb(false);
    });
}
...
function getDatabaseList(cb) {
    ...
    getClient('test', function(err, testDb) {
        ...
        adminDb.listDatabases(function(err, dbs) {
            ...
            logger.debug('Database list size: ' + dbs.length + ', Entries: ', JSON.stringify(dbs));
