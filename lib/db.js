...
//The prefix can change depending on the environment we are running in.  Some of
//this is old and can probably be refactored/removed.
if (config.env.docker) {
    dbURLPrefix = 'mongodb://' + config.env.docker.db + '/';
}

if (process.env.RUNTIME_ENV === 'container') {
    dbURLPrefix = 'mongodb://' + config.env.hosts.db + '/';
}

//If DB_URI is set, it overrides the others because it means we're running on AWS
//and pointing to a cloud-based version of Mongo Atlas.
if (process.env.DB_URI) {
    dbURLPrefix = process.env.DB_URI;
    console.log('DB_URI detected');
    // if (dbURLPrefix.charAt(dbURLPrefix.length - 1) !== '/') {
    //     dbURLPrefix += '/';
    // }
}
