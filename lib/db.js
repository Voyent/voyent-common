...
//The dbURL is used to connect to the database. The name of the
...
let dbURL = config.database.scheme + '://' + config.database.host + ':' + config.database.port;
let dbOptions = {};

/**
 * For local development or running on a basic Cloud environment, the
 * dbURL and dbOptions are pretty simple.  When running in a more
...
function determineDBConnectionSettings() {
    ...
    if (process.env.DB_URI) {
        ...
        //Generate a prefix we can use going forward.  Start with the scheme
        dbURL = parsedDBURI.scheme + '://';

        //Add username and password if they exist.
        if (parsedDBURI.username && parsedDBURI.password) {
            dbURL = dbURL +
    ...
    console.log('dbURL', JSON.stringify(dbURL, null, 4));
    console.log('dbOptions', JSON.stringify(dbOptions, null, 4));
}

determineDBConnectionSettings();

//Using the shared client approach where all databases use a single connection pool, then you need
//to have an initial client.
let sharedClient;
...
function testCollectionExistence(accountName, collectionName, cb) {
    ...
    let safeDB = getSafeDatabaseName(accountName);
        ...
        adminDb.listDatabases(function(err, dbs) {
            ...
            getClient(safeDB, function(err, db) {
                ...
                db.listCollections( {} ).toArray(function(err, items) {
                    ...
                    collectionName + ' in: ' + collectionList);
                    return cb(false);
                });
            });
        });
...
function testDatabaseExistence(accountName, cb) {
    getAccountRecord(accountName, function(err, account) {
        cb(account != null);
    })
}
 ...
 */
let getAccountRecord = function (accountName, cb) {
    let logger = require('./loggers').getLogger();
    let safeName = getSafeDatabaseName(accountName);

    testCollectionExistence('auth', 'accounts', function(doesExist) {

        if (!doesExist) {
            return cb(new errors.ServerError('accountNotFound'), null);
        }
        getCollection('auth', 'accounts', function(err, collection) {
            collection.findOne({safeAccountname: safeName}, {}, function (err, account) {
                if (err) {
                    logger.error('Exception finding account record in admin.accounts collection', err);
                    return cb(err, null);
                }
                // In doing logging cleanup, this is a bigger error and should be logged; here, the account collection
                // exists but is missing the account record
                if (!account || !account.accountname) {
                    logger.warn('Database: ['+safeName+ '] contains admin.accounts collection, but it is empty?');
                    return cb(new errors.ServerError('accountNotFound'), null);
                }
 ...
 * @param safeAccountName the safe account name
 * @param cb callback
 */
...
function getRealmCollections(accountName, realmName, callback) {
    ...
    getCollections(safeDB, function (err, items) {
        ...
        let collList = [];
        for (let i = 0; i < items.length; i++) {
            let spos = items[i].name.toLowerCase().indexOf(realmName.toLowerCase());
            if (spos === 0) {
...
function getClient(accountName, cb) {
    if (!sharedClient) {
        //see https://github.com/mongodb/node-mongodb-native/releases/tag/v3.2.1
        ...
        MongoClient.connect(dbURL, dbOptions, function (err, client) {
            ...
            let db = sharedClient.db(accountName);
            return cb(null, db);
        });
    } else {
        let db = sharedClient.db(accountName);
 ...
 * @param strict       true if strict mode is to be applied
 * @param cb            callback of form (err, collection)
 */
function getCollectionWithOptions(dbName, collectionName, options, cb) {
    let logger = require('./loggers').getLogger();
    stats.collectionRequests += 1;

    return getClient(dbName, function (dbErr, db) {
        if (dbErr) {
            logger.error('problem getting client', dbErr);
            return cb(dbErr, null);
        }

        db.collection(collectionName, options, function (collErr, coll) {
            if (collErr) {
            ...
            //created.
            db.createCollection(collectionName, {w: 1}, function (createErr, newCollection) {
                if (createErr) {
 ...
 * @returns {*}
 */
function getCollection(dbName, collectionName, cb) {
    return getCollectionWithOptions(dbName, collectionName, {strict: false}, cb);
}
 ...
 * @returns {*}
 */
function createIndexedCollection(dbName, collectionName, options, cb) {
    let logger = require('./loggers').getLogger();
    stats.collectionRequests += 1;
    ...
    return getClient(dbName, function (dbErr, db) {
        ...
        db.createCollection(collectionName, options, function (createErr, newCollection) {
            if (createErr) {
                logger.error('could not create collection', createErr);
                return cb(createErr, null);
            }
...
function getCappedCollection(accountName, realmName, collectionName, capsize, cb) {
    let logger = require('./loggers').getLogger();
    stats.collectionRequests += 1;
    let capSize = capsize || 100000;

    return getClient(accountName, function (dbErr, db) {
        if (dbErr) {
            logger.error('problem getting client', dbErr);
            return cb(dbErr, null);
        }
...
function getConnectionCurrentStrategy(accountName, callback) {
     getClient(accountName, callback);
}
