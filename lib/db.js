...
function testCollectionExistence(accountName, collectionName, cb) {
    ...
    getClient('test', function(err, testDb) {
        ...
        adminDb.listDatabases({nameOnly: true}, function(err, dbs) {
            if (err) {
...
function testDatabaseExistence(accountName, cb) {
    ...
    if (accountCache[safeDB]) {
        logger.debug('Account:', safeDB, 'found in cache');
        return cb(true);
    } else {
        logger.debug('Account:', safeDB, 'NOT found in cache, looking the old way...');
    }
    // If not found in the cache, do the expensive lookup
    getDatabaseList(function (err, dbs) {
        for (var i = 0; i < dbs.length; i++) {
            if (dbs[i].name == safeDB) {
                accountCache[safeDB] = true;
                return cb(true);
            }
        }
        if (dbs.length == 0) {
...
function getCollectionWithOptions(accountName, realmName, collectionName, options, cb) {
    ...
    return getClientStrategy(accountName, function (dbErr, db) {
        ...
        var fullCollectionName = realmName.toLowerCase() + '.' + collectionName;
...
function createIndexedCollection(accountName, realmName, collectionName, options, cb) {
    ...
    return getClientStrategy(accountName, function (dbErr, db) {
        ...
        const fullCollectionName = realmName + '.' + collectionName;
