...
let dbURLPrefix = config.database.scheme + '://' + config.database.host + ':' + config.database.port;
let dbOptions = {};
...
let getClientStrategy = getClientShared;
...
function testCollectionExistence(accountName, collectionName, cb) {
    ...
    getClientStrategy('test', function(err, testDb) {
        if (err) {
        ...
        adminDb.listDatabases(function(err, dbs) {
            ...
            getClientStrategy(safeDB, function(err, db) {
                if (err) {
...
function getCollections(safeAccountName, cb) {
    ...
    getClientStrategy(safeAccountName, function(err, db) {
        if (err) {
            logger.error('Exception getting database connection: ' + JSON.stringify(err));
            return cb(false);
        }
...
function dropDatabase(accountName,  cb) {
    getClientStrategy(accountName, function(err, db) {
        if (err) {
            return cb(err, false);
        }
...
module.exports.getClient = getClientShared;
...
function getCollectionWithOptions(dbName, collectionName, options, cb) {
    ...
    return getClientStrategy(dbName, function (dbErr, db) {
        if (dbErr) {
