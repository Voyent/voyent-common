...
let dbURLPrefix = config.database.scheme + '://' +
...
function determineDBConnectionSettings() {
    ...
    if (process.env.DB_URI) {
        ...
        dbURLPrefix = dbURLPrefix + hosts.join() + '/';
...
// the set of account databases. If an accountname is present, it's a valid account.
// If a test
var accountCache = {};
    ...
    getClient('test', function(err, testDb) {
        ...
        adminDb.listDatabases(function(err, dbs) {
            ...
            getClient(safeDB, function(err, db) {
                ...
                db.listCollections( {}, {nameOnly: true} ).toArray(function(err, items) {
                    if (err) {
...
function getClient(rawAccountName, cb) {

    var logger = require('./loggers').getLogger();

    const accountName = getSafeDatabaseName(rawAccountName);
    ...
    if (currentlyProcessing[accountName]) {
        ...
        MongoClient.connect(dbURL, dbOptions, function (err, client) {
            ...
            let db = client.db(accountName);
            db.close = function() {
                client.close(false, function(err) {
                    if (err) {
                        logger.error('Failed to close database ' + accountName, err);
                    }
                });
            };
...
function createIndexedCollection(dbName, collectionName, options, cb) {
    ...
    return getClient(dbName, function (dbErr, db) {
        ...
        db.collection(collectionName, {strict: false}, function (collErr, coll) {
            if (collErr) {
                logger.error('problem getting reference to collection', collErr);
                return cb(collErr, null);
            }
