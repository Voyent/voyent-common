...
function resolveLocalStackClients() {
    ...
    clients.s3 = new AWS.S3({endpoint: "http://aws:4566"});
    clients.sns = new AWS.SNS({endpoint: "http://aws:4566"});
    clients.sqs = new AWS.SQS({endpoint: "http://aws:4566"});
    clients.awsRegion = region;

    console.log('AWS region (from env) is [' + region + ']');
...
function resolveOfficialClients(cb) {
    ...
    agent.get(url)
        .end(function (err, response) {
            ...
            region = ld.get(parsedReponse, 'region');
            ...
            console.log('AWS region (from identity file) is [' + region + ']');
