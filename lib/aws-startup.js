    ...
    async.each(
        ...
        function (configFile, acb) {
            ...
            getBucketObject(bucketName, configFile, function (err, data) {
                ...
                if (err) {
                    console.log('aws-startup','problem getting bucket object', bucketName, configFile, err);
                    acb(err);
                    return;
                }
...
function loadEnvironmentProperties(cb) {
    ...
    async.waterfall([
        ...
        //Try to get something we know is there.
        function (wcb) {
            getBucketObject(bucketName, 'common.config', wcb);
        },
