...
function createQueue(topicName, queueName, cb) {
    ...
    async.waterfall([
        ...
        function (queueName, acb) {
            ...
            awsClient.sqs.createQueue(params, acb);
        },
        ...
        function (dlqArn, acb) {
            ...
            awsClient.sqs.setQueueAttributes(params, acb);
        }
...
function createDeadLetterQueue(cb) {
    ...
    getQualifiedQueueName(topicName, queueName, function (err, qName) {
        ...
        awsClient.sqs.createQueue(params, function (err, data) {
            ...
            getQueueArn(topicName, queueName, function (err, queueArn) {
                ...
                awsClient.sqs.setQueueAttributes(params, function (err) {
...
function deleteQueue(topicName, queueName, cb) {
    ...
    getQueueUrl(topicName, queueName, function (err, qURL) {
        ...
        awsClient.sqs.deleteQueue(params, function (err, data) {
            if (err) {
                cb(err);
                return;
            }
...
function createTopic(topicName, cb) {
    ...
    getQualifiedTopicName(topicName, function (err, tName) {
        ...
        awsClient.sns.createTopic(params, function (err, data) {
            ...
            if (publishParams) {
                ...
                awsClient.sns.addPermission(publishParams, function (permissionErr, permData) {
...
function deleteTopic(topicName, cb) {
    ...
    queuedGetTopicArn(topicName, function (err, topicArn) {
        ...
        awsClient.sns.deleteTopic(params, function (err, data) {
...
function getAllTopics(cb) {
    ...
    async.doWhilst(
        function (acb) {
            ...
            awsClient.sns.listTopics(params, function (err, data) {
...
function getQueueUrl(topicName, queueName, cb) {
    ...
    getQualifiedQueueName(topicName, queueName, function (err, qName) {
        ...
        awsClient.sqs.getQueueUrl(params, function (err, data) {
            if (err) {
                cb(err);
                return;
            }
...
function getQueueArn(topicName, queueName, cb) {
    ...
    getQueueUrl(topicName, queueName, function (err, qURL) {
        ...
        awsClient.sqs.getQueueAttributes(params, function (err, data) {
...
function publishMessageToTopic(topicName, message, attrs, cb) {
    ...
    queuedGetTopicArn(topicName, function (err, topicArn) {
        ...
        awsClient.sns.publish(params, function (err, data) {
...
function sendMessageToQueue(topicName, queueName, message, cb) {
    ...
    getQueueUrl(topicName, queueName, function (err, queueUrl) {
        ...
        awsClient.sqs.sendMessage(params, function (err, data) {
...
function getSubscriptionsForTopic(topicArn, cb) {
    ...
    awsClient.sns.listSubscriptionsByTopic(params, function (err, data) {
        if (err) {
            cb(err);
            return;
        }
    ...
    async.parallel({
        ...
        checkQueueAlreadySubscribed(
            ...
            function (err, sub) {
                ...
                awsClient.sns.subscribe(params, function (err, data) {
    ...
    async.parallel({
        ...
        checkQueueAlreadySubscribed(
            ...
            function (err, sub) {
                ...
                awsClient.sns.setSubscriptionAttributes(params, function (err, data) {
...
function publishAppUpdateEvent(account, realm, username, typeInPayload, txcode, groups, fullPayload) {
    ...
    // Need to ensure our groups are account/realm scoped
    if (account && realm) {
        var acctRealm = account + realm;
        for (let groupLoop = 0; groupLoop < officialGroups.length; groupLoop++) {
            if (officialGroups[groupLoop].indexOf(acctRealm, officialGroups[groupLoop].length - acctRealm.length) === -1) {
                officialGroups[groupLoop] = officialGroups[groupLoop] + acctRealm;
            }
        }
    }
