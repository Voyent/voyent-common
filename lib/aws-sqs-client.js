...
function SQSClient() {
    ...
    function connect() {
        ...
        awsClient.getClients(function (err, clients) {
            ...
            if (err) {
                logger.debug('problem getting AWS SQS client', err);
                return;
            }

            clients.sqs.receiveMessage(params, responseHandler);
        });
    }
    ...
    function changeMessageVisibility(handle, timeout) {
        ...
        awsClient.getClients(function (err, clients) {
            ...
            if (err) {
                logger.error('problem getting AWS SQS client', params, err);
                return;
            }

            clients.sqs.changeMessageVisibility(params, function (err) {
                if (err) {
    ...
    function deleteMessage(handle) {
        ...
        awsClient.getClients(function (err, clients) {
            ...
            if (err) {
                logger.error('problem getting AWS SQS client', err);
                return;
            }

            clients.sqs.deleteMessage(params, function (err) {
    ...
    function responseHandler(err, response) {
        ...
        if (response) {
            ...
            if (response.Messages && response.Messages.length > 0) {
                ...
                event.setVisibility = function (secs) {

                    if(!secs || !ld.isNumber(secs)){
                        logger.warn(
                            'event.setVisibility',
                            'message visibility is not a number',
                            secs,
                            firstMessage.ReceiptHandle
                        );
                        return;
                    ...
                    //This is the min and max (12 hours) supported by AWS
                    if (!ld.inRange(secs, 0, 43200)) {
                        logger.warn(
                            'event.setVisibility',
                            'message visibility not in range (0-43200)',
                            secs,
                            firstMessage.ReceiptHandle
                        );
                        return;
                    ...
                    logger.debug(
                        'event.setVisibility',
                        'setting message visibility for', secs,
                        firstMessage.ReceiptHandle
                    );

                    changeMessageVisibility(firstMessage.ReceiptHandle, secs);
                };
...
function listSubscriptionsForTopic(topicArn, nextToken, cb) {
    ...
    awsClient.getClients(function (err, clients) {
        ...
        clients.sns.listSubscriptionsByTopic(params, function (err, results) {
                ...
                async.each(params.matchingSubscriptions,
                    function (sub, ecb) {
                        ...
                        awsClient.getClients(function (err, clients) {
                            ...
                            clients.sns.unsubscribe(unsubscribeParams, function (err, result) {
