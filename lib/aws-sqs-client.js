    ...
    function responseHandler(err, response) {
        ...
        if (response) {
            ...
            if (response.Messages && response.Messages.length > 0) {
                ...
                //By default, we are deleting the message as soon as we've
                ...
                //service instance to handle.  We'll look for an env variable
                //that will turn off this behaviour as desired.  This should
                //be removed completely once the services are all handling
                //the messages in a more granular fashion.
                const deleteMessages = process.env.AWS_DELETE_MESSAGES || 'always';

                if (deleteMessages === 'always') {
                ...
                try {
                    ...
                    if (firstMessage.ReceiptHandle) {
                        ...
                        //eventually be resolved, call event.failure() with an
                        ...
                        //eventually be resolved, call event.suspend() with an
                        //optional parameter for the number of secs of message
                        ...
                        event.suspend = function (secs) {
                            ...
                            let suspendedFor = secs || 30;

                            logger.debug(
                                'suspending message', firstMessage.ReceiptHandle,
                                'visibility for', suspendedFor);
