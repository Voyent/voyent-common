...
function getOrigin(req, callback) {
    ...
    if (req.headers['x-forwarded-host']) {
        ...
        if (rawOrigin) {
            logger.info('X1/3) Origin vaue from forwarded host: ' + rawOrigin);
        }
    }
    ...
    if (rawOrigin) {
        if (typeof(rawOrigin) !=='string') {
            logger.info('Type of origin value is not string: ' + typeof(rawOrigin));
        ...
        if (spos != -1) {
            sanitizedOrigin = sanitizedOrigin.substring(spos+3);
        }
        var testRegEx = /^[A-Za-z0-9\.\-\$_]*$/;
        if (!testRegEx.test(sanitizedOrigin)) {
            logger.info('Origin regex is failing on: ' + sanitizedOrigin);
            return callback(new errors.BadRequestError('invalidOriginFormat'), null);
        }
    }
