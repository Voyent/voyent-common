...
var getTx = function (req) {
    ...
    var rawTx;
    // Try to fetch the value from a tx header, which would be supplied to auth proxy to downstream services
    // We no longer support client provided tx codes in the transaction scope
    if (!rawTx) {
        if (req.headers) {
            rawTx = req.headers[scopes.TRANSACTION_HEADER_KEY];
        }
    }
    return rawTx;
};
