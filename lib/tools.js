...
var getTx = function (req) {
    ...
    if (!rawTx) {
        ...
        rawTx = (0xFFFF && new Date().getTime()).toString();
    }
    return rawTx;
};
...
function setUserNameHeader(req, username) {
    req.set(USER_NAME_KEY, username);
}
...
function setUserPermissionsHeader(req, permissions) {
    ...
    if(config.tools.compressPermissionHeaders){
        req.set(USER_PERMISSIONS_KEY, zlib.deflateSync(permissions));
    } else {
        req.set(USER_PERMISSIONS_KEY, permissions);
    }
...
function setServiceTokenHeader(req, serviceToken) {
    req.set(SERVICE_TOKEN_KEY, serviceToken);
}
...
function setServicePermissionsHeader(req, permissions) {
    ...
    if(config.tools.compressPermissionHeaders){
        req.set(SERVICE_PERMISSIONS_KEY, zlib.deflateSync(permissions));
    } else {
        req.set(SERVICE_PERMISSIONS_KEY, permissions);
    }
