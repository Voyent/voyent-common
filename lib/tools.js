...
/**
 * If INTERSERVICE_HOST_MODE is to anything but "aws", then the
 * getInterserviceHost() function will just return the same value it was passed.
 * If set to "aws", then getInterserviceHost() will append:
 *
 *     ".ENVIRONMENT_NAME"
 *
 * to the incoming service name.
 *
 * @type {string | string}
 */
const INTERSERVICE_HOST_MODE = process.env.INTERSERVICE_HOST_MODE || "aws";

/**
 * When services make direct HTTP calls to other services, the environment we
 * are running in is important.  If we are running locally or on AWS with the
 * old Weave infrastructure, we use:
 *
 *     serviceName:port (e.g. auth:55010)
 *
 * When running on AWS without Weave, the host is in the form of:
 *
 *      serviceName.environmentName:port (e.g. auth.demo2:55010)
 *
 * So this convenience method just takes the incoming service name and adjusts
 * it if required for the environment.  The setting of the following environment
 * variables impacts the behaviour of this function:
 *
 *   INTERSERVICE_HOST_MODE: If INTERSERVICE_HOST_MODE is to anything but "aws",
 *                           then the getInterserviceHost() function will just
 *                           return the same value it was passed. If set to "aws",
 *                           then getInterserviceHost() will append
 *                           ".ENVIRONMENT_NAME" to the incoming service name.
 *
 *   ENVIRONMENT_NAME: This specifies the environment we are running in - will
 *                     be something like local, demo1, scaling2.  The value is
 *                     appended to the service name when running in AWS without
 *                     Weave.
 *
 * @param serviceName
 * @returns {string}
 */
function getInterserviceHost(serviceName) {

    let host = serviceName;

    if (INTERSERVICE_HOST_MODE === 'aws') {
        host = serviceName + '.' + process.env.ENVIRONMENT_NAME;
    }

    const logger = require('./loggers').getLogger();
    logger.debug('getInterServiceHost', INTERSERVICE_HOST_MODE, serviceName, host);
    return host;
}

module.exports.getInterserviceHost = getInterserviceHost;
...
module.exports.getServicePort = getServicePort;


/**
 *  This function takes a service name, and constructs the proper
 *  scheme://host:port/service
 *
 *  In the case of service='authadmin', the code looks up the host for 'auth' but appends the 'authadmin' service
 *  In the case of service='authInternal', the code looks up the host for 'auth' but appends the 'authInternal' service
 */
function getInternalHostPath(service) {
    ...
    let workingService = service;  // capture the original service. Including the change to docs
    if (service === 'authadmin') {
        service = 'auth';
    }
    if (service === 'authInternal') {
        service = 'auth';
    }

    // service is now the key to the port and host, but workingService is the original service path
    let port = getServicePort(service);
    if (!port) {
        return;
    }

    return 'http://' + getInterserviceHost(service) + ':' + port +
        '/' + workingService;

}

module.exports.getInternalHostPath = getInternalHostPath;
...
const USER_PERMISSIONS_KEY = 'com.icesoft.services.auth.user.permissions';
...
const SERVICE_PERMISSIONS_KEY = 'com.icesoft.services.auth.service.permissions';
...
var generateServiceStartupEvent = function (serviceName, serviceRecord) {
    ...
    setTimeout(function () {
        ...
        ac.publishServiceEventFromBarrel(barrel);
...
var getTokenExpiry = function (environmentVariableName) {
    ...
    if (environmentVariableName) {
        try {
            ...
            if (returnVal < 5000) {
                returnVal = (returnVal < 5000) ? config.defaultAccessTokenExpiry : returnVal;
            }
        } catch (e) {
            return config.defaultAccessTokenExpiry;
        }
    }
    return returnVal;
};
