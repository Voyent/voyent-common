...
function getBaseMetric(barrel) {
    ...
    var theAccount = barrel.account;
    if(!theAccount){
        theAccount = barrel.accountId;
        if(!theAccount){
            logger.warn('metric being sent without a valid account');
        }
    }
    ...
    var data = {
        ...
        username: barrel.username || barrel.permissionCheck.username,
    ...
    if (barrel.validatedResource && barrel.validatedResourceId) {
        data.resourceId = barrel.validatedResourceId;
    }
...
function sendToMetricsService(record) {
    ...
    var options = {
        uri: config.metrics.url + '/' + record.data.account + '/realms/' + record.data.realm + '/stats',
