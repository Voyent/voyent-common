...
/**
 * This handles the HTTP call to the Auth service to get a context's access_token.  The
 * credentials should be of the form: {
 ...
 * }
 *
 * The callback will return an error or the auth service response containing the access_token property
 * @param credentials object containing account, realm, username, and password properties with salient details
 */

function requestContextToken(credentials, cb) {

    var logger = require('./loggers').getLogger();
    var token = TOKEN_CACHE[credentials.username];
    var expired = true;
    if (token) {
        var expireCheck = token.expires_at - new Date().getTime();
        if (expireCheck < 0) {
            logger.debug('Found TOKEN has expired, refetching: ' + credentials.username);
        } else {
            logger.debug('CACHED Token found for: --> ' + credentials.username + ', expires in -> ' + expireCheck + 'ms');
            return cb(null, token);
        }
    } else {
        logger.debug('requestContextToken, No Token in cache');
    }

    if (!expired) {
        return cb(null, token);
    } else {
        TOKEN_CACHE[credentials.username] = undefined;
    }
    ...
    var uri = btools.constructInternalHostPath('auth');
    var options = {
        uri: uri +  '/' + credentials.account + '/realms/' + credentials.realm + '/token',
        method: 'GET',
        headers: headers,
        qs: queryParams
    };
    ...
    httpClient.get(options, function (err, response) {
        ...
        if (err) {
            logger.error('problem with context token request:', err);
            return cb(err, null);
        }
        ...
        if (response) {
            ...
            if (response.statusCode < 200 || response.statusCode > 299) {
                ...
                try {
                    result = btools.getBodyAsJSON(response);
                    logger.warn('problem getting service token (error from auth):' + result.status, result.message);
                    return cb( new errors.PermissionError(result.message) );
                } catch (e) {
                    logger.warn('Exception parsing token response: ' + response.statusCode);
                    logger.warn('Can occur on non-JSON response from passport, or potential other sources. Content Follows: ');
                    logger.warn(response.body);
                    return cb(new errors.OperationError(response.statusCode, response.body));
                }
            } else {
                try {
                    ...
                    logger.debug('Raw Response to Context Token request: ', response.body);
                    var result = btools.getBodyAsJSON(response);
                    result['expires_at'] = new Date().getTime() + result['expires_in'];
                    TOKEN_CACHE[credentials.username] = result;
                    cb(null, result);
...
module.exports.requestContextToken = requestContextToken;
