...
/**
 * Get the entire contents of the _data partition in the resource permission environment
 * The client will not see the structure of the data, only the original record structure
 */
function getStructuredResource(barrel, cb) {
    ...
    barrel.collection.find(barrel.query, barrel.fieldsFilter, barrel.queryOptions).limit(1).toArray( function (err, result) {
...
function updateStructuredResource(barrel, cb) {
    ...
    if (barrel.subDocumentFlag) {
        updateObject['_data.' + barrel.resourceType] = barrel.validatedResource[barrel.resourceType]; // the value of the subdoc part only
    } else {
        updateObject['_data'] = barrel.validatedResource; // The whole object
    }
        ...
        barrel.collection.updateOne(barrel.query,
            {
                ...
                logger.debug('xxUpdate diagnostics: docs scanned: ' + doc.result.n + ', docs modified: ' +
...
function incrementStructuredResourceArray(barrel, cb) {
    ...
    // All client information is stored in the _data partition. Setting a further namespace string
    // will allow us to be more specific with the increment
    var possibleRecordCount = 0;

    var updateObject = {};
    updateObject['_data.' + barrel.resourceType] = barrel.validatedResource; // the validated resource is the update
    ...
    barrel.collection.count(barrel.countQuery, function (err, count) {
        ...
        logger.debug('Maximum increment record count: ' + count);
        if (count === 0) {
            return cb(new errors.NotFoundError('resourceNotFound'), barrel);
        }
        possibleRecordCount = count;
        barrel.collection.updateOne(barrel.query,
            {
                ...
                logger.debug('xxIncrement diagnostics: docs scanned: ' + doc.result.n + ', docs modified: ' +
                              doc.result.nModified);
                if (possibleRecordCount != doc.result.nModified) {
                    return cb(new errors.PermissionError('permissionNotGranted'), barrel);
                }
