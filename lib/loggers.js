...
module.exports.getLogger = function getLogger(userOptions) {
    ...
    if (!winston.loggers.has(settings.logName)) {
        ...
        var auditConfig = {
            name: 'auditTransport',
            level: config.auditLoggerSettings.level,
            collection: config.auditLoggerSettings.collection,
            fixedRealm: config.auditLoggerSettings.fixedRealm, // This is kept in a fixed 'audit' realm
            capSize: config.auditLoggerSettings.capSize
        };

        var debugConfig = {
            name: 'debugTransport',
            level: config.debugLoggerSettings.level,
            collection: config.debugLoggerSettings.collection,
            capSize: config.debugLoggerSettings.capSize,
            fixedRealm: config.debugLoggerSettings.fixedRealm  //
        };
