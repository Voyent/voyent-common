...
//Information for communicating with the Mailbox service.
config.bewhere = {};
config.bewhere.scheme = 'http';
config.bewhere.host = config.env.host === LOCAL_HOST ? LOCAL_HOST : config.env.proxyHost;
config.bewhere.port = config.env.host === LOCAL_HOST ? ':55160' : '';
config.bewhere.path = '/bewhere';
config.bewhere.url = config.bewhere.scheme + '://' + config.bewhere.host + config.bewhere.port + config.bewhere.path;
...
// Set the debug Collection = 'void' to turn off the debugging logging
config.debugLoggerSettings = {
    collection: 'debug',
    capSize: 10000  // 10000 records per realm
};
