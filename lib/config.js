...
// Additional notification costing default cost structure. All prices in whatever the front end units are
config.defaultCostSchedule = {
    prepaidUserCount:   500,         // (will vary by user)
    additionalUserRate: 1.05,        // Cost per additional user over the prepaid user amount in the customer’s plan (e.g. $1.05).
    androidNotificationRate: 0.0,    // Cost per Android native notification sent (e.g. $0.00).
    iosNotificationRate: 0.00,       // Cost per iOS native notification sent (e.g. $0.00).
    smsSegmentRate: 0.04,            // Cost per SMS segment sent (e.g. $0.04).
    emailNotificationRate: 0.02,     // Cost per email notification sent (e.g. $0.02).
    voiceMinuteRate: 0.06,           // Cost per voice dial call minute used to deliver voice notifications (e.g. $0.06).
    facebookRate: 0.0,               // Cost per facebook notification
    twitterRate: 0.0                 // Cost per twitter notification
};
