 ...
 *
 * @param indexVal The index record to be used
 * @param cb a callback of the form (err, collection)
 */
var applyIndexFunction = function (indexVal, cb) {

    var logger = require('./loggers').getLogger();

    db.getCollection(indexVal.accountId, indexVal.realmId, indexVal.collectionName, function(err, coll) {
        if (err) {
            return cb(err);
        }
        indexCollection(coll, indexVal, function (err, collName) {
            if (err) {
                logger.error('Exception applying Index: ', indexVal);
            }
            logger.debug('in applyIndexFunction callback, got Collection? ', (coll!= undefined) );
            this.collection = coll;

            if (indexVal.data) {
                coll.insertMany( indexVal.data, { }, function (insertErr, result) {
