 ...
 * key/value store. This function checks a permission against the set included in the transaction scope
 *
 * @param barrel
 * @param cb
 */
...
/**
 * Get a collection from fields defined in the barrel:
 * - barrel.accountId,
 * - barrel.realmId,
 * - barrel.collectionName
 *
 * This function supports an optional index object in barrel.collectionIndex which can take the form:
 * collectionIndex : { "_id": 1, "_scopeId": 1, etc}
 *
 * NOTE! This index mechanism does NOT work with Capped or TTL indexes in conjunction with other indexes.
 * If you want to cap or TTL a collection, or have any doubts, use applyIndexesToCollection instead.
 *
 * The original use of this function was to get a collection, for which it can still be used.
 *
 *
 * @param barrel
 * @param cb
...
function getCollection(barrel, cb) {
    ...
    db.getCollection(barrel.accountId, barrel.realmId, barrel.collectionName, function (err, coll) {
        ...
        // CreateIndex should be used if the index doesn't exist, ensureIndex checks if the
        // index exists to avoid duplicate indexes.
        if (barrel.collectionIndex) {
            coll.createIndex(barrel.collectionIndex, function (indexError, indexName) {
                if (indexError) {
...
/**
 * This function is the template for all index application calls. For each of the
 * index records in the index array, this function will be bound to the barrel and the index Record.
 * In the code, the value of 'this' is the barrel
 *
 * @param indexRecord The index record to be used
 * @param cb a callback of the form (err, collection)
 */
let applyIndexFunction = function (indexRecord, cb) {

    let logger = require('./loggers').getLogger();

    logger.debug('applyIndexFunction', JSON.stringify(indexRecord));

    let collectionOptions = {};

    if (indexRecord.capSize) {
        collectionOptions.capped = true;
        collectionOptions.max = indexRecord.capSize;
    }

    if (indexRecord.maxSize) {
        collectionOptions.capped = true;
        collectionOptions.size = indexRecord.maxSize;
    }

    db.createIndexedCollection(
        indexRecord.accountId,
        indexRecord.realmId,
        indexRecord.collectionName,
        collectionOptions,
        function (err, coll) {
            ...
            if (err && err.message && err.message.indexOf('already exists') != -1) {
                logger.debug('> ^^^ The Unmaskable exception above: Collection:', indexRecord.collectionName, 'already exists');
                logger.debug('!> Is OK <!, continuing to apply any indexes provided');
                const fullName = indexRecord.accountId + '.' + indexRecord.realmId + '.' + indexRecord.collectionName;
                logger.debug(fullName, 'already exists');
                params.exists = true;
            } else if (err) {
                logger.error('Exception creating index:', err);
                return cb(err);
            } else {
            ...
            async.waterfall([
                function (icb) {
                    ...
                    // If Collection already exists, fetch it in order to apply the index
                    if (params.exists) {

                        db.getCollection(indexRecord.accountId, indexRecord.realmId, indexRecord.collectionName, function (err, coll) {
                            if (err) {
                                logger.error('Exception fetching collection:', indexRecord.collectionName, err);
                                return icb(err, params);
                            }
                            params.collection = coll;
                            return icb(null, params);
                        });
                ...
                function (params, icb) {
                    ...
                    //Only index if provided
                    if (!indexRecord.collectionIndex) {
                        return icb(null, params);
                    }

                    //Always index in the background so we hold things up as
                    //little as possible.
                    let indexOptions = {
                        background: true
                    };

                    if (indexRecord.unique) {
                        indexOptions.unique = indexRecord.unique;
                    }

                    //See if there is an expiry setting for the index.
                    let expires = indexRecord.expireAfter || indexRecord.expireAfterSeconds;
                    if (expires) {
                        indexOptions.expireAfterSeconds = expires;
                    }

                    // vras-917 Allow clients to apply case insensitive collation index
                    if (indexRecord.collation) {
                        indexOptions.collation = indexRecord.collation;
                        indexOptions.v = 2;
                        indexOptions.unique = true;
                    }
                    // vras-1687 Apply the index name if present. It will default to a value, but
                    // if it needs to be unique it needs to be applied
                    if (indexRecord.name) {
                        indexOptions.name = indexRecord.name;
                    }

                    params.collection.createIndex(
                        indexRecord.collectionIndex,
                        indexOptions,
                        function (indexErr, indexResult) {

                            //If there is a problem we'll log it but we won't
                            //stop the entire async indexing process because of it.
                            if (indexErr) {
                                logger.warn('problem creating index', indexErr);
                                return icb(indexErr, params);
                            }

                            logger.debug(
                                'Created index',
                                JSON.stringify(indexRecord),
                                JSON.stringify(indexOptions)
                            );
                ...
                function (params, icb) {
                    ...
                    //Only insert data if provided
                    if (!indexRecord.data) {
                        return icb(null, params);
                    }

                    params.collection.insertMany(indexRecord.data, {}, function (insertErr, insertResult) {

                        if (insertErr) {
                            logger.error('problem inserting data', insertErr);
                            icb(insertErr, params);
                            return;
                        }
            ...
            ], function (aErr, params) {

                if (aErr) {
                    logger.error('problem creating indexed collection', aErr);
                    return cb(aErr);
                }
                cb();
            });
...
/**
 * This function goes through the barrel and runs through an array of index Descriptors to
 * apply any desired indexes to the collection, and secondly, insert any initial records into the
 * collection
 *
 * @param barrel
 * @param cb
 */
var processCollectionInitialization = function (barrel, cb) {

    var logger = require('./loggers').getLogger();

    // fill in an array with function instances bound to the runner.
    var idx = 0;
    var indexExecFunctions = [];
    ld.forEach(barrel.com_icesoft_indexArray, function (indexVal) {
        var boundExec = applyIndexFunction.bind(barrel, indexVal);
        indexExecFunctions[idx++] = boundExec;
    });

    logger.debug('processCollectionInitialization Found: ' + indexExecFunctions.length + ' Index records to apply');
    // async is going to run these collection functions for us. Series please, so we
    // don't run into illegal access errors
    async.series(indexExecFunctions, function (indexError, indexResults) {
        if (indexError) {
            logger.error('Exception creating indexedCollection: ', indexError);
            return cb(indexError, barrel);
        }

        return cb(indexError, barrel);
    });
};
module.exports.processCollectionInitialization = processCollectionInitialization;
...
function normalizeResourceId(barrel, cb) {
    ...
    if (officialResourceId) {
        ...
        barrel.validatedResourceId = officialResourceId.toString();

        //It must be a 24 character hex string to have a chance of being an official MongoDB _id
        //and not just a custom String.
        if (/^[0-9a-fA-F]{24}$/.test(barrel.validatedResourceId)) {
            barrel.searchQuery = {_id: ObjectID.createFromHexString(barrel.validatedResourceId)};
        } else {
            barrel.searchQuery = {_id: barrel.validatedResourceId};
        }
    }
...
function findResourceViaResourceQuery(barrel, cb) {
    ...
    if (barrel.resultsScope === 'all') {
        ...
        barrel.collection.find(barrel.query, barrel.queryOptions).project(barrel.fieldsFilter).limit(1).toArray(function (err, results) {
...
/**
 * This function takes standard properties from the barrel:
 * barrel.username,
 * barrel.roles,
 * barrel.ream
 *
 * and creates the standard query permissions structure. This can be used in either the raw query or in the
 * aggregate decoration functions
 *
 * @param barrel
 */
function createPermissionsQuery(barrel) {

    var logger = require('./loggers').getLogger();

    if (!barrel.operation) {
        logger.error('Coding error, call to decorateQuery without required operation parameter', barrel.meta);
        return undefined;
    }
    var rolePhraseBlock = [];
    for (var rn in barrel.roles) {
    ...
    if (rolePhraseBlock.length > 0) {
        ...
        logger.debug('No roles found, skipping role phrases');
    }
    barrel.query = {
        $or: topLevelOrClauses,   //resource permission query
        $and: [{}]                 //user query
...
module.exports.createPermissionsQuery = createPermissionsQuery;


/**
 * Decorate a query with the permissions phrases. The query is passed as an argument and the info required to
 * add to the phrases can come from standard locations in the barrel. This updates the barrel.query location
 * as well as returning the new query.
 * @param barrel
 * @param clientQuery The pure query phrase requiring decoration
 */
function decorateQueryWithPermissions(barrel, clientQuery) {

    var logger = require('./loggers').getLogger();
    var workingQuery = createPermissionsQuery(barrel);
    ...
    // permissions clauses
    if (clientQuery) {
        for (var i in clientQuery) {
            barrel.countQuery[i] = clientQuery[i];
        }
    }
    workingQuery.$and[0] = barrel.countQuery;
    barrel.query = workingQuery;
    return barrel.query;
}
module.exports.decorateQueryWithPermissions = decorateQueryWithPermissions;

/**
 * Calculate a new $match query phrase for aggregation given an existing query by calculating the permissions
 * clauses and merging them with the original query. This does not decorate the query in the existing location
 *
 * @param barrel
 * @param clientQuery The Entire aggregate block
 */
function calculateAggregateWithPermissions(barrel, clientAggregate) {

    var logger = require('./loggers').getLogger();

    var permissionClauses = createPermissionsQuery(barrel);
    var clientQuery = clientAggregate.query[0];
    var aggregateMatch = clientQuery; // default

    if (clientQuery) {
        // There will only be one clause, the $match clause. This is effectively the
        // count clause. All the query clauses are already embedded into the $match string
        for (var i in clientQuery) {
            if (i === '$match') {
                barrel.countQuery = clientQuery[i];
                aggregateMatch = {
                    $match: {
                        $and: [
                            clientQuery[i],
                            {$or: permissionClauses.$or}
                        ]
                    }
                };
            }
...
module.exports.calculateAggregateWithPermissions = calculateAggregateWithPermissions;
...
function aggregate(barrel, cb) {
    ...
    barrel.collection.aggregate(barrel.aggregation).toArray(function (err, results) {
...
function getPermissionsDirectory(barrel, cb) {
    ...
    barrel.collection.find(barrel.query, barrel.queryOptions).project(barrel.fieldsFilter).toArray(function (err, files) {
        if (err) {
...
module.exports.processResourceMetadata = processResourceMetadata;


// Fetch context token
var fetchAvatarToken = function (barrel, cb) {

    var logger = require('./loggers').getLogger();

    var actionContext = barrel.com_icesoft_avatar_context;
    var credentials = {};
    credentials.account = barrel.req.params.accountId;
    credentials.realm = barrel.req.params.realmId;
    credentials.username = actionContext.username;
    credentials.password = actionContext.password;

    authCall.requestContextToken(credentials, function (err, response) {

        if (err) {
            logger.error('Exception in AvatarToken read', barrel.meta);
            return cb(err, barrel);
        }

        barrel.com_icesoft_tokenStorage = response.access_token;
        return cb(null, barrel);
    });
};
module.exports.fetchAvatarToken = fetchAvatarToken;


var lookupAvatarCredentials = function (barrel, cb) {

    var logger = require('./loggers').getLogger();
    var actionId = barrel.req.params.resourceId;
    var now = new Date().getTime();

    // Get Context credentials from user database, currently
    var account = db.getSafeDatabaseName(barrel.req.params.accountId);
    var realm = barrel.req.params.realmId.toLowerCase();

    db.getCollection(account, realm, 'users', function (err, coll) {
        if (err) {
            logger.error("Exception getting User Collection ", err, barrel.meta);
            return cb(err, barrel);
        }

        coll.find({username: actionId}).toArray(function (err, actionContext) {
            if (err) {
                logger.error("Exception getting AvatarContext User: ", barrel.meta);
                return cb(null, barrel);
            }
            if (actionContext.length != 1) {
                logger.error('AvatarContext search found wrong number of results: ', actionContext.length);
                return cb(new errors.ServerError('contextNotFound'), barrel);
            }

            actionContext[0].createdAt = now; // Add an expiry time for later cache cleanup
            barrel.com_icesoft_avatar_context = actionContext[0];
            return cb(null, barrel);

        });
    });
};
module.exports.lookupAvatarCredentials = lookupAvatarCredentials;

/**
 * Update the transaction scoped permissions with the contents of the avatar context record
 * Context record is stored in com_icesoft_avatar_context
 */
var updateTransactionPermissions = function (barrel, cb) {

    var logger = require('./loggers').getLogger();
    var avatarRecord = barrel.com_icesoft_avatar_context;

    var perms = avatarRecord.permissions;
    if (!perms) {
        perms = [];
    }
    var roles = avatarRecord.roles;
    if (!roles) {
        roles = [];
    }
    var gotResult = barrel.transactionData;
    logger.debug('Retrieved payload from tx scope, tx: ' + barrel.meta.tx + ', Payload:', JSON.stringify(gotResult));
    gotResult.username = avatarRecord.username;
    gotResult.permissions = perms;
    gotResult.roles = roles;
    gotResult.access_token = barrel.com_icesoft_tokenStorage;


    logger.debug('Updating Transaction txPayload with context criteria');
    logger.debug('New Permissions: ', JSON.stringify(perms));
    scopes.setTransactionAttribute(barrel, 'txPayload', gotResult).then(
        function (setResult) {
            cb(null, barrel);
        }
    ).catch(
        function (err) {
            logger.error('Exception updating permissions in tx scope: ', err, barrel.meta);
            return cb(err, barrel);
        }
    );
};
module.exports.updateTransactionPermissions = updateTransactionPermissions;
