...
function isValidCollectionName(collectionName) {
    ...
    if (ld.isUndefined(collectionName) ||
        ld.isEmpty(collectionName) ||
        !(ld.isString(collectionName)) ||
...
function processQueryIdParameter(barrel, cb) {
    ...
    if (barrel.req.query && barrel.req.query.queryId) {
        ...
        var parsedURL = url.parse(config.query.url + '/' + barrel.accountId + '/realms/' + barrel.realmId + '/' + barrel.req.query.queryId);
    ...
    if (barrel.req.query) {
        ...
        if (barrel.req.query.query) {
            try {
                ...
                barrel.searchQuery = ld.omit(theQuery, '__proto__');
            } catch (parseError) {
                var errorMessage = {
                    parameter: 'query',
    ...
    if (barrel.req.query) {
        ...
        if (barrel.req.query.fields) {
            try {
                ...
                barrel.fieldsFilter = ld.omit(theFields, '__proto__');
            } catch (parseError) {
                var errorMessage = {
                    parameter: 'fields',
                    message: 'could not parse',
    ...
    if (barrel.req.query) {
        ...
        if (barrel.req.query.options) {
            try {
                ...
                barrel.queryOptions = ld.omit(theOptions, '__proto__');
            } catch (parseError) {
                var errorMessage = {
                    parameter: 'options',
                    message: 'could not parse',
...
function updateResource(barrel, cb) {
    ...
    if (!barrel.excludeId) {
        if (barrel.searchQuery && !barrel.searchQuery._id &&
            barrel.validatedResourceId && !barrel.validatedResource._id) {
            barrel.validatedResource._id = barrel.validatedResourceId;
        }
    }
