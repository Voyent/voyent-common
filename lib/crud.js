...
var applyIndexFunction = function (indexRecord, cb) {
    ...
    logger.debug('Applying index to collection: ' + indexRecord.accountId + ' -> ' + indexRecord.realmId + '.' + indexRecord.collectionName);
    var options = {};

    if (indexRecord.expireAfter) {
        options.expireAfterSeconds = indexRecord.expireAfter;
    }
    if (indexRecord.capSize) {
        options.capped = true;
        options.max = indexRecord.capSize;
    }
    if (indexRecord.maxSize) {
        options.capped = true;
        options.size = indexRecord.maxSize;
    }

    db.getIndexedCollection(indexRecord.accountId, indexRecord.realmId, indexRecord.collectionName, options, function(err, coll) {
        if (err) {
            return cb(err);
        }
        this.collection = coll;
        if (indexRecord.data) {
            coll.insertMany( indexRecord.data, { }, function (insertErr, result) {

                if (insertErr) {
                    logger.error('Error bulk inserting data: ', insertErr, this.meta);
                    return cb(insertErr);
                }
                return cb(null);
            });
...
/**
 * This function can be called to ensure an index of any particular type on a collection.
 * This function must be called multiple times for multiple collections
 *
 * @param barrel
 * @param cb
 */
function indexCollection(collection, indexRecord, cb) {

    var logger = require('./loggers').getLogger();
    logger.debug('Applying index to collection: ' + indexRecord.accountId + ' -> ' + indexRecord.realmId + '.' + indexRecord.collectionName);

    if (!collection) {
        logger.error('Must pass in a pre-existing collection');
        return cb(new errors.ServerError('generalError'));
    }
    var options = {};
    ...
    if (indexRecord.capSize) {
        options.capped = true;
        options.size = indexRecord.capSize;
    }
    if (indexRecord.maxSize) {
        options.capped = true;
        options.max = indexRecord.maxSize
    }
    // Apply any capping collections
    collection.ensureIndex(indexRecord.collectionIndex, options, function (indexError, indexName) {
        if (indexError) {
            logger.error('could not create capped index: ' + indexError);
            cb(indexError, null);
            return;
        }
        logger.debug('Capped index applied: ' + indexName);
        cb(null, indexName);
    });
}
