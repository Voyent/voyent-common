...
var prefixPhrases = function(phraseBlock) {
    var prefixPhrase = function (phrase) {
        us.each(phrase,function (value, key, list) {
            ...
            //only prefix when we have a named key (obj prop not array index) that is not a query operator ($and, $ne, etc..) and not '_id'
            if (us.isString(key) && key.indexOf('$') == -1 && key != '_id') {
