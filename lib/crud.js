...
function findResourceViaResourceQuery(barrel, cb) {
    ...
    if (barrel.resultsScope === 'all') {
        ...
        barrel.collection.findOne(barrel.query, barrel.fieldsFilter, barrel.queryOptions, function (err, results) {
            ...
            barrel.results = !newResults ? {} : newResults._data;
            cb(null, barrel);
        });
    }
}
 ...
 * resource with the contents of the update
 */
function updateStructuredResource(barrel, cb) {
    ...
    barrel.collection.update(barrel.searchQuery,
        {
...
var prefixPhrases = function(phraseBlock) {
    var queryBlock = {};
    for (var i in phraseBlock) {
        var clause = phraseBlock[i];
        queryBlock[dataBinPrefix+i] = clause;
    }
    return queryBlock;
};
