...
function verifyPermission(barrel, cb) {
    ...
    logger.debug('verifying permission, config.service.proxy.enabled:', config.service.proxy.enabled);
    ...
    //logger.debug('username header:', barrel.username);

    barrel.userPermissions = btools.getUserPermissionsHeader(barrel.req);
    if (!barrel.userPermissions) {
        cb(new errors.AuthorizationError('requestMissingPermissions'));
        return;
    }
    //logger.debug('user permissions header:', barrel.userPermissions);

    //Check if the service requires a specific static permission.  If so, make
    //sure the permission headers set by the Auth Proxy include it.
    if (barrel.permission) {

        //Verify that the permission is set in the header.  If not
        //the request is rejected.
        if (!btools.hasUserPermissionInHeader(barrel.req, barrel.permission)) {
    ...
    if (barrel.dynamicPermission) {
        ...
        //No resource id, no permission.
        if (!barrel.req.params || !barrel.req.params.resourceId) {
            cb(new errors.BadRequestError('requestMissingResource'));
            return;
        }

        var dynamicPermission = barrel.dynamicPermission + barrel.req.params.resourceId;

        //Verify that the permission is set in the header.  If not
        //the request is rejected.
        if (!btools.hasUserPermissionInHeader(barrel.req, dynamicPermission)) {
            cb(new errors.AuthorizationError('permissionNotGranted'));
            return;
        }
    }
...
function checkPermission(barrel, cb) {
    ...
    //If we are using the Auth Proxy, we don't exercise this function.
    //Instead, we call the verifyPermission function which is designed
    //to work with the Auth Proxy.
    if(config.service.proxy.enabled){
...
function retrieveServiceToken(barrel, cb) {
    ...
    authCall.retrieveServiceToken(params, function (err, token) {
