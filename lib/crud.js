...
// The prefix portions of resource metadata query clauses
var metaRolePrefix = 'metadata.resourceMetadata.permissions.rights.roles.';
var clientMetadataPrefix = 'metadata.clientMetadata.';
 ...
 * }
 * @param barrel.operation one of 'r', 'u', 'd', 'x'
...
function assembleResourceQuery(barrel, cb) {
    ...
    var topLevelOrClauses = [
        {
            'metadata.resourceMetadata.permissions.owner': barrel.username,
            'metadata.resourceMetadata.permissions.rights.owner': barrel.operation
        },
        {
            'metadata.resourceMetadata.permissions.rights.realm': barrel.operation
        }
    ];
    ...
    // The count query allows us to determine the number of records the client 'could' access. If that
    // returns 0, then we generate a 404 error.
    barrel.countQuery = {};
    if (barrel.resourceId) {
        barrel.query['metadata.resourceMetadata.resourceId'] = barrel.resourceId;
        barrel.countQuery['metadata.resourceMetadata.resourceId'] = barrel.resourceId;
    }

    // Add in the client metadata clauses for good measure
    var clientClause;
    if (barrel.searchQuery) {
        for (var i in barrel.searchQuery) {
            clientClause =  clientMetadataPrefix + i;
            barrel.query[clientClause] = barrel.searchQuery[i];
            barrel.countQuery[clientClause] = barrel.searchQuery[i];
        }
    }
    cb(null, barrel);
}
