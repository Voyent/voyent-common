...
/**
 * Fetch the value kept in the redis keystore for the transaction code stored in the
 * request tx header. Parses the value, and parses out the values into the barrel
 */
function getTransactionData(barrel, cb) {
    var logger = require('./loggers').getLogger();
    var redisConfig = process.env.DOCKER_LOCAL? config.env.docker.redis: config.env.hosts.redis;

    logger.error('_____ redisConfig: ' + redisConfig);

    var tx = "";
    if (barrel.req.headers) {
        tx = barrel.req.headers['tx'];
    }
    if (!tx) {
        return cb(new errors.BadRequestError('transactionNotFound'), barrel);
    }
    btools.getRedisClient(redisConfig, function (err, rc) {
        logger.debug('Getting value from redis: ' + tx);
        rc.get(tx, function (err, payload) {
            if (err) {
                logger.error('err !! ' + err, barrel.meta);
                return cb(err, barrel);
            }
            logger.debug('Redis service response: ' + payload + ', typeof Redis response: ' + typeof(payload), barrel.meta);
            if (!payload) {
                logger.error('Missing transaction code payload');
                return cb(new errors.BadRequestError('transactionNotFound'), barrel);
            }

            var reply = JSON.parse(payload);

            barrel.transactionPayload = reply._data;
            barrel.username = reply._data.username;
            barrel.access_token = reply._data.access_token;
            barrel.permissions = reply._data.permissions;
            barrel.username = reply._data.username;
            barrel.roles = reply._data.roles;
            cb(null, barrel);
        });
    });
}
