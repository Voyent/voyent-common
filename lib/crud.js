...
function getCollection(barrel, cb) {
    ...
    var logger = require('./loggers').getLogger();
    logger.debug('getting database collection, operation: ' + barrel.eventName + ', collection:' + barrel.accountId + ' -> ' + barrel.collectionName);

    db.getCollection(process.env.SERVICE_NAME, barrel.collectionName, function (err, coll) {
        if (err) {
            logger.error('could not get collection: ' + barrel.meta.service + ' -> ' + barrel.realmId + '.' + barrel.collectionName, err);
...
let applyIndexFunction = function (indexRecord, cb) {
    ...
    db.createIndexedCollection(
        indexRecord.serviceName,
        indexRecord.collectionName,
        collectionOptions,
        function (err, coll) {

            if (err) {
                logger.error('problem creating indexed collection', err);
                cb(err);
                return;
            }

            logger.debug('created collection', JSON.stringify(indexRecord));
            ...
            async.series([
                function (acb) {
                    ...
                    this.collection.createIndex(
                        ...
                        function (indexErr, indexResult) {
                            ...
                            //stop the entire async process because of it.
                            if (indexErr) {
                                logger.error('problem creating index', indexErr);
                            }
                ...
                function (acb) {
                    ...
                    this.collection.insertMany(indexRecord.data, {}, function (insertErr, insertResult) {
...
function findAllResources(barrel, cb) {
    ...
    barrel.query = {
        _account: barrel.accountId,
        _realm: barrel.realmId
    };
...
/**
 * Remove all resources in a collection. Checks on permissions are done outside this function now
 * that resource permissions are gone.
 */
function removeAllResources(barrel, cb) {
    ...
    barrel.collection.remove({}, {w: 1}, function (err, writeOpResult) {
...
/**
 * This will change in the future. This will not allow the client to define the _id,
 *
 ...
 *   properties from barrel.validatedResource
 * }
 */
function insertStructuredResource(barrel, cb) {
    ...
    newRecord._account = barrel.accountId;
    newRecord._realm = barrel.realmId;
    newRecord._id = barrel.validatedResource._id;
...
function updateStructuredResource(barrel, cb) {
    ...
    barrel.collection.update(barrel.query, barrel.validatedResource
        , barrel.queryOptions, function (err, numberOfDocs) {
            if (err) {
                logger.error('Error updating resource: ' + JSON.stringify(err), barrel.meta);
                return cb(err, barrel);
            }
...
function assembleResourceQuery(barrel, cb) {
    ...
    var topLevelAndClauses =
    {
        '_account': barrel.accountId,
        '_realm': barrel.realmId
    };

    barrel.query = topLevelAndClauses;

    // Add in all the previous query clauses in a block for 'and' ing with the
    // permissions clauses
    if (barrel.searchQuery) {
        for (var i in barrel.searchQuery) {
            barrel.query[i] = barrel.searchQuery[i];
        }
    }
    cb(null, barrel);
}
...
function updatePermissionsResource(barrel, cb) {
    ...
    return cb( new errors.ServerDetailsError('serverError', 'updatePermissionsResource no longer supported'),
                barrel);
...
function lookupAvatarCredentials(barrel, cb) {
    var logger = require('./loggers').getLogger();
    ...
    var params = {
        access_token: barrel.access_token
    };
    var username = barrel.req.params.resourceId;
    var account = barrel.accountId;
    var realm = barrel.realmId;
    var options = {
        uri: btools.getInternalHostPath('authInternal') + '/' + account + '/realms/' + realm + '/lookup-avatar-credentials/' + username,
        headers: headers,
        qs: params
    };
    ...
    httpClient.get(options, function (err, response) {
        ...
        if (response.statusCode >= 200 && response.statusCode < 299) {
            ...
            try {
                ...
                barrel.com_icesoft_avatar_context = result;
                return cb(null, barrel);
            } catch (e) {
