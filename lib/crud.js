...
var applyIndexFunction = function (indexRecord, cb) {
    ...
    db.createIndexedCollection(indexRecord.accountId, indexRecord.realmId, indexRecord.collectionName, options, function(err, coll) {
        if (err) {
            logger.error('Exception indexing new collection: ', err);
            return cb(err);
        }
        logger.debug('- Index successfully applied to collection: ' + indexRecord.accountId + ' -> ' + indexRecord.realmId + '.' + indexRecord.collectionName);
