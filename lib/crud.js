...
/**
 * @deprecate Should be replaced with the records and structure to do resource permissions
 */
...
function findResourceViaResourceQuery(barrel, cb) {
    ...
    // For the standard find operations, do not return the _permissions object
    barrel.fieldsFilter['_permissions'] = 0;
    ...
    if (barrel.resultsScope === 'all') {
        ...
        barrel.collection.findOne(barrel.query, barrel.fieldsFilter, barrel.queryOptions, function (err, results) {
...
function removeResourceViaResourceQuery(barrel, cb) {
    ...
    logger.debug('removing resources with: ' +
        '\n  query   = ' + JSON.stringify(barrel.query)
    );

    barrel.collection.remove(barrel.query, {w: 1}, function (err, numberOfDocs) {
        ...
        //Only complain if trying to delete a specific resource
        if (numberOfDocs === 0 && barrel.searchQuery._id) {
            cb(new errors.NotFoundDetailsError('resourceNotFound', JSON.stringify(barrel.query)));
            return;
        }
        barrel.results = numberOfDocs;
        cb(null, barrel);
...
function assembleResourceQuery(barrel, cb) {
    ...
    barrel.username = btools.getUserNameHeader(barrel.req);
    barrel.roles = btools.getRoleHeader(barrel.req);
 ...
 * @param cb
 */
function updatePermissionsWithQuery(barrel, cb) {
    ...
    if (barrel.rights) {
        ...
        barrel.collection.count(barrel.countQuery, function (err, count) {
                ...
                barrel.collection.update(barrel.query,
                    {
                        $set: {"_permissions.rights": barrel.rights}
                    },
                    function (err, doc) {
                        if (err) {
                            logger.error('Error updating resource metadata: ' + JSON.stringify(err), barrel.meta);
                        }
                        // todo: This can fail if 'count()' finds a single record, but update() does not
                        logger.debug('Mismatch in permission update possible: ' + possibleRecordCount +
                        ', actually updated: ' + doc);

                        if (doc != possibleRecordCount && possibleRecordCount === 1) {
                            return cb(new errors.PermissionError('permissionNotGranted'), barrel);
                        } else if (doc != possibleRecordCount) {
                            // todo: attempt to update multiple records, but only had permissions to do some
                            // Can't fail, since some updates have passed. Can't pass back status since
                            // 204 precludes return values...
                        }
                        cb(err, barrel);
                    });
            });
...
exports.updatePermissionsWithQuery = updatePermissionsWithQuery;
...
/**
 * This function should be used by clients to create a new resourceMetadata object. It starts with the
 * default resourceMetadata block from an applications config module, then fetches a 'rights' metadata object
 ...
 */
var processRights = function(barrel, cb) {
