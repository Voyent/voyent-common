...
function adjustNotificationForFCM(notification, userInfo) {
    ...
    ld.set(notification, 'cloud.fcm.data.zoneId', ld.get(notification, 'zoneId'));
...
function buildCloudPayload(params, userInfo) {
    ...
    let isVoice = userInfo.notificationEndpoint.startsWith('voice');
    if (isVoice) {
        let fromPhoneNumber = ld.get(params,'fromPhoneNumber');
        if (fromPhoneNumber) {
            ld.set(record, 'voice.from', fromPhoneNumber);
        }
    }
...
function buildBroadcastPayload(params, userInfo) {
    ...
    if (!params.skipLogs) {
        ld.set(record, 'payload.nid', userInfo.nid);
    }
...
function buildNotificationPayloads(params) {
    ...
    let payloadParams = {
        ...
        fromPhoneNumber: ld.get(params, 'accountInfo.custom.fromPhoneNumber')
    };
...
function getAccountInfo(params, cb) {
    db.getCollection('auth', 'accounts',
        function (err, coll) {
            ...
            const query = {safeAccountname: params.accountId};

            coll.findOne(query, function (err, accountInfo) {

                if (err) {
                    logger.error(
                        'problem finding admin record',
                        params.accountId,
                        'admin',
                        collName,
                        err.message
