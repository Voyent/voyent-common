...
function buildCloudPayload(params) {
    ...
    ld.set(record, 'timezoneOffset', params.timezoneOffset);
...
function buildBroadcastPayload(params) {
    ...
    if (params.alertId) {
        ld.set(record, 'payload.global.alertId', params.alertId);
    }
    if (params.alertFamilyId) {
        ld.set(record, 'payload.global.alertFamilyId', params.alertFamilyId);
    }
    if (params.badge) {
        ld.set(record, 'payload.global.badge', params.badge);
    }
...
function requestNotifications(params, cb) {
    ...
    async.waterfall([
        ...
        function (acb) {
            ...
            context.emptyPayloads = ld.get(context, 'payloads.length', 0) === 0;
            // logger.debug('requestNotifications', 'payloads: ', JSON.stringify(context.payloads));
            acb();
        },

        function (acb) {
            if(context.emptyPayloads){
        ...
        if (err) {
            logger.debug('requestNotifications', 'error', err);
            return cb(err);
        }
