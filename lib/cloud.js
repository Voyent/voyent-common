 ...
 * each one and saves them all to the database.
 *
 * @param params
 ...
 * @param cb
 */
function createNotificationLogs(params, cb) {

    logger.debug('createNotificationLogs', params.accountId, params.realmId, params.users);
    ...
    //Create a notification log for each username/notificationEndpoint entry.
    let newLogs = ld.map(params.users, function (user) {

        //Check to protocol of the endpoint to avoid creating a notification log
        //for bad ones.
    ...
    params.notificationLogs = ld.compact(newLogs);

    db.getCollection('action', 'notificationLog', function (err, coll) {
            if (err) {
                logger.error(
            ...
            coll.insertMany(params.notificationLogs, function (err) {
                if (err) {
                    return cb(err);
                }
                cb();
 ...
 * @param notification
 */
function adjustNotificationForAPNS(notification) {
 ...
 * @param notification
 */
function adjustNotificationForFCM(notification) {

    ld.set(notification, 'cloud', {
        silent: true,
        fcm: {
            data: notification.cloud
        }
    });
    ld.unset(notification, 'cloud.detail');
    ld.unset(notification, 'sms');
    ld.unset(notification, 'email');
    ld.unset(notification, 'voice');
}
 ...
 * @param notification
 */
function adjustNotificationForEmail(notification) {
 ...
 * @param notification
 */
function adjustNotificationForSMS(notification) {
 ...
 * @param notification
 */
function adjustNotificationForVoice(notification) {
 ...
 */
function adjustNotificationForProtocol(notification, notificationEndpoint) {

    let protocol =
        notificationEndpoint.substring(0, notificationEndpoint.indexOf(':'))
            .trim()
            .toLowerCase();

    switch (protocol) {
        case 'apns':
            adjustNotificationForAPNS(notification);
            break;
        case 'fcm':
            adjustNotificationForFCM(notification);
            break;
        case 'mailto':
            adjustNotificationForEmail(notification);
            break;
        case 'sms':
            adjustNotificationForSMS(notification);
            break;
        case 'voice':
            adjustNotificationForVoice(notification);
            break;
        default:
            throw new Error('unknown protocol', notificationEndpoint);
    }

}


function buildMailboxPayload(params) {
    ...
    ld.set(record, 'message.group', params.notificationEndpoint);
    ld.set(record, 'message.issuer', params.issuer);
    ld.set(record, 'message.nid', params.nid);
...
function buildBroadcastPayload(params) {
    ...
    ld.set(record, 'payload.global.groupId', params.username);
    ld.set(record, 'payload.global.issuer', params.issuer);
    ld.set(record, 'payload.global.nid', params.nid);
    ld.set(record, 'payload.global.timezone', params.timezone);
    ld.set(record, 'payload.global.timezoneCode', params.timezoneCode);
    ld.set(record, 'payload.global.type', params.type);

    return record;
}


function buildServicesPayloads(params) {
    ...
    if (!params.services || params.services.indexOf('broadcast') >= 0) {
        payload.broadcast = buildBroadcastPayload(ld.cloneDeep(params));
    }

    if (!params.services || params.services.indexOf('cloud') >= 0) {
        payload.cloud = buildCloudPayload(ld.cloneDeep(params));
    }

    if (!params.services || params.services.indexOf('mailbox') >= 0) {
        payload.mailbox = buildMailboxPayload(ld.cloneDeep(params));
    }

    logger.debug(
        'buildServicesPayloads',
        params.notificationEndpoint,
...
function buildNotificationPayloads(params) {
    ...
    return ld.map(params.users, function (user) {

        //Add the user specific information to the payload params.
        payloadParams.username = user.username;
        payloadParams.notificationEndpoint = user.notificationEndpoint;
        payloadParams.nid = user.nid;
        payloadParams.notification = ld.cloneDeep(params.notification);

        //Adjust the notification to the users notification protocol.  Logs
        //an error if the protocol is unknown.
        try {
            ...
            user.notification =
                adjustNotificationForProtocol(
                    ...
                    payloadParams.notificationEndpoint
                );

            return buildServicesPayloads(payloadParams);
...
function saveNotificationPayload(params, payload, serviceName, cb) {
    ...
    db.getCollection(serviceName, 'batch', function (err, coll) {
        payload._account = params.accountId;
        payload._realm = params.realmId;
 ...
 *                                             of th message. This is optional
 *                                             and defaults to 10.
 *                                             10 = informational
 *                                             20 = critical
 *                                             30 = immediate
 ...
 *
 * @param {string} [params.type=general] - The type of the message.  Can be
 *                                         whatever you want.  It's optional and
 *                                         defaults to "general".
 *
...
function requestNotifications(params, cb) {
    ...
    async.waterfall([
        ...
        function (acb) {
            ...
            context.issuer = context.realmInfo.displayName || 'Voyent Alert!';
        ...
        function (acb) {
            ...
            createNotificationLogs(context, acb);
        },

        function (acb) {
            context.payloads = ld.compact(buildNotificationPayloads(context));
            logger.debug('requestNotification', 'payloads: ', JSON.stringify(context.payloads));
            acb();
        },
